...
type Props = {
  ...
  onRetry: () => void
  /** Give up and go back to the start screen. */
  onDismiss: () => void
  /**
   * A failure during the call (a microphone that cut out, say) is a banner you
   * can dismiss; a failure to connect at all replaces the conversation.
   */
...
export function ConnectionNotice({ error, onRetry, onDismiss, variant }: Props) {
  ...
  return (
    ...
    >
      <Eyebrow>{blocking ? "Couldn't start" : 'Problem'}</Eyebrow>
...
/** Shown while the room and the agent are still coming up. */
