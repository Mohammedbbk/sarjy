...
export function parseProposal(raw: Record<string, unknown>): ProposalInput | null {
  ...
  if (typeof actionId !== 'string' || !uuid(actionId) || typeof entryId !== 'string' || !uuid(entryId) ||
      typeof issueId !== 'string' || !uuid(issueId)) return null
  if (kind === 'comment' && typeof body === 'string' && body.trim().length > 0 && body.trim().length <= 500 && targetStatus === undefined) {
    return { actionId, entryId, issueId, kind, body: body.trim() }
  }
  if (kind === 'status' && typeof targetStatus === 'string' && targetStatus.trim().length > 0 &&
      targetStatus.trim().length <= 80 && body === undefined) {
    return { actionId, entryId, issueId, kind, targetStatus: targetStatus.trim() }
  }
  return null
}

export async function proposeAction(visitorId: string, standupId: string, input: ProposalInput, gateway: LinearWriteGateway = linearWriteGateway): Promise<Result> {
  ...
  if (previous.data) return previous.data.standupId === standupId
    ? { ok: true, action: previous.data } : fail(409, 'That proposal ID is already in use.')
  let issue: Awaited<ReturnType<LinearWriteGateway['issue']>>
  try { issue = await gateway.issue(input.issueId) }
  catch (error) { return fail(error instanceof LinearTargetError ? 400 : 503, error instanceof LinearTargetError ? error.message : 'Could not check the Linear ticket.') }
  const target = input.kind === 'status'
    ? issue.states.find((state) => state.name.toLowerCase() === input.targetStatus!.toLowerCase()) : undefined
  ...
  const saved = await saveProposal({
    visitorId, standupId, actionId: input.actionId, entryId: input.entryId,
    issueId: issue.id, issueIdentifier: issue.identifier, issueTitle: issue.title, issueUrl: issue.url,
    kind: input.kind, body: input.kind === 'comment' ? input.body! : null,
    fromStateId: input.kind === 'status' ? issue.stateId : null,
    fromStateName: input.kind === 'status' ? issue.stateName : null,
    toStateId: target?.id ?? null, toStateName: target?.name ?? null,
  })
  if (!saved.ok) return fail(503, 'Could not save the proposal.')
  if ((saved.data.status === 'ok' || saved.data.status === 'replayed') && saved.data.action) return { ok: true, action: saved.data.action }
...
async function reconcile(action: LinearAction, gateway: LinearWriteGateway): Promise<Outcome> {
  ...
  } catch { return { status: 'uncertain', message: 'Could not verify the outcome in Linear.' } }
}

async function execute(action: LinearAction, gateway: LinearWriteGateway): Promise<Outcome> {
  let issue: Awaited<ReturnType<LinearWriteGateway['issue']>>
  try { issue = await gateway.issue(action.issueId) }
  catch (error) { return { status: 'failed', message: error instanceof LinearTargetError ? error.message : 'Could not check the Linear ticket.' } }
  if (action.kind === 'status') {
    if (issue.stateId !== action.fromStateId) return { status: 'failed', message: 'The ticket status changed since this proposal. Ask Sarjy for a new one.' }
    if (!issue.states.some((state) => state.id === action.toStateId)) return { status: 'failed', message: 'The proposed status is no longer available.' }
  }
  try {
  ...
  } catch { return reconcile(action, gateway) }
...
export async function confirmAction(visitorId: string, actionId: string, gateway: LinearWriteGateway = linearWriteGateway): Promise<Result> {
...
export async function checkAction(visitorId: string, actionId: string, gateway: LinearWriteGateway = linearWriteGateway): Promise<Result> {
